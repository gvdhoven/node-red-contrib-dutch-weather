...
class WeatherLogic extends EventEmitter {
	...
	/**
	 * Creates a rainprediction object
	 *
	 * @param date/time
	 ...
	 *
	 * @param object Prediction to send if it is different from last state change
	 */
	rainPredictionUpdate(rainPrediction) {
		var emitRainPrediction = (isEqual(this.rainState, rainPrediction) ? false : true);
		if (emitRainPrediction === true) {
			this.rainState = rainPrediction;
	...
	async checkRain(after) {
		...
		for (let i = 0; i < 23; i++) {
			...
			try {
				...
				var stateAtTime = this.rainPredictionPayload(atTime, inMinutes, buienradarPrecipitation.mmh, buienalarmPrecipitation.mmh);
				//console.log(stateAtTime);
				if (inMinutes == 0) {
		...
		this.rainPredictionUpdate(tempState);
	}
