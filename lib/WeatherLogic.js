...
class WeatherLogic extends EventEmitter {
  ...
  iso8601dateTime(dt) {
    ...
    if (current_millis == 0) current_millis = "000"
    else if (current_millis > 0 && current_millis < 10)
      current_millis = "00" + current_millis
    else current_millis = "0" + current_millis
  ...
  rainStatePayload(atTime, inMinutes, buienRadar, buienAlarm) {
    ...
    let msg =
      probability == 100 ? "" : "There is a " + probability + "% chance that "
    if (state === "dry") {
  ...
  rainStateUpdate(rainState, force) {
    let emitRainState =
      force || (isEqual(this.rainState, rainState) ? false : true)
  ...
  async checkRain(force) {
    ...
    for (let i = 0; i < 23; i++) {
      ...
      try {
        ...
        this.emit(
          "rain-error",
          "Failed getting prediction for Buienradar for time " +
            atTime.toISOString() +
            ": " +
            JSON.stringify(e)
        )
      ...
      try {
        ...
        this.emit(
          "rain-error",
          "Failed getting prediction for Buienalarm for time " +
            atTime.toISOString() +
            ": " +
            JSON.stringify(e)
        )
      ...
      stateAtTime = this.rainStatePayload(
        atTime,
        inMinutes,
        buienradarPrecipitation,
        buienalarmPrecipitation
      )
      ...
      if (
        !tempState.hasOwnProperty("prediction") &&
        tempState.now.state !== stateAtTime.state
      ) {
    ...
    if (tempState.hasOwnProperty("now")) {
      ...
      let dt = this.iso8601dateTime(atTime)
      let msg =
        tempState.prediction.probability == 100
          ? ""
          : "There is a " + tempState.prediction.probability + "% chance that "
      if (tempState.prediction.state === "dry") {
        msg +=
          (tempState.prediction.inMinutes > 0 ? "it will be" : "it is") + " dry"
      } else {
        msg +=
          (tempState.prediction.inMinutes > 0 ? "there will be" : "there is") +
          " " +
          tempState.prediction.state
      }

      if (tempState.prediction.inMinutes > 0) {
        msg +=
          tempState.now.state === tempState.prediction.state
            ? " for at least "
            : " in "
  ...
  async updateMeteoplaza(force) {
    ...
    if (newMeteoplaza !== null) {
      haveUpdate =
        oldMeteoplaza === null ||
        (oldMeteoplaza !== null &&
          newMeteoplaza.lastFetched > oldMeteoplaza.lastFetched)
