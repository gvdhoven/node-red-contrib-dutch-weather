...
class WeatherLogic extends EventEmitter {
	...
	async checkRain(after) {
		...
		this.emit('state', this.rainState, buienradarPrecipitation.mmh, buienalarmPrecipitation.mmh);
		...
		for (let i = 0; i < 8; i++) {
			...
			let atTime = new Date(after.getTime() + inMinutes * 60000)

			// Get forecasts for both providers
			let buienradarPrecipitation = await this.Buienradar.getForecast(atTime);
			let buienalarmPrecipitation = await this.Buienalarm.getForecast(atTime);
			...
			if (this.rainState === false && this.rainStartTriggered === false) {
				...
				if (averagePrecipitation >= RAIN_LIGHT) {
					...
					this.emit('prediction', true, inMinutes.toString(), buienradarPrecipitation.mmh, buienalarmPrecipitation.mmh);
			...
			if (this.rainState === true && this.rainStopTriggered === false) {
				...
				if (averagePrecipitation < RAIN_LIGHT) {
					...
					this.emit('prediction', false, inMinutes.toString(), buienradarPrecipitation.mmh, buienalarmPrecipitation.mmh);
