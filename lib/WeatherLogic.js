...
class WeatherLogic extends EventEmitter {
	...
	startMonitor() {
		...
		clearInterval(this.solarEventsTimer);
		clearInterval(this.rainTimer);
		clearInterval(this.meteoplazaTimer);

		// Update sun events
		this.sunPositionTimer = setInterval(this.updateSunPosition.bind(this), MINUTE);

		// Update sun position
		this.solarEventsTimer = setInterval(this.updateSolarEvents.bind(this),  10 * MINUTE);
	...
	async checkRain(after) {
		...
		if (tempState.hasOwnProperty('now')) {
			this.rainStateUpdate(tempState);
		}
	}
