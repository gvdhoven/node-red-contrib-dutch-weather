...
module.exports = class Buienalarm {
  ...
  async update() {
    ...
    if (this.forecasts.lastFetched != null) {
      getNewForecast =
        (now.getTime() - this.forecasts.lastFetched.getTime()) / 1000 >= MAX_AGE
    }
    ...
    while (true) {
      try {
        // Fetch new data
        let url = `https://cdn-secure.buienalarm.nl/api/3.4/forecast.php?lat=${
          this.lat
        }&lon=${this.lng}&region=nl&unit=mm/u&rnd=${new Date().getTime()}`
