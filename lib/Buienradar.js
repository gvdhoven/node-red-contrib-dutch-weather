...
module.exports = class Buienradar {
	...
	async update() {
		...
		if (this.forecasts.lastFetched != null) {
			getNewForecast = (((now.getTime() - this.forecasts.lastFetched.getTime()) / 1000) >= 300);
		}
		...
		while (true) {
			try {
				// Fetch new data
				let url = `https://gpsgadget.buienradar.nl/data/raintext?lat=${this.lat}&lon=${this.lng}&rnd=${(new Date()).getTime()}`;
