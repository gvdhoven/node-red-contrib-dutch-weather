...
module.exports = class Meteoplaza {
  ...
  async update() {
    ...
    if (this.forecast.lastFetched != null) {
      var rounded = new Date(
        Math.ceil(this.forecast.lastFetched.getTime() / 604800) * 604800
      )
    ...
    try {
      // Fetch new data
      var url = `https://api.meteoplaza.com/v2/meteo/completelocation/${
        this.lat
      }.${this.lng}?lang=nl&rnd=${new Date().getTime()}`
      ...
      if (
        json == null ||
        !json.hasOwnProperty("Astro") ||
        !json.hasOwnProperty("Hourly")
      ) {
        return null
      }
      ...
      json.Hourly.forEach((hour) => {
        if (hour.ValidDt.substring(0, 10) == today) {
          ...
          newData.today.tempMin =
            newData.today.tempMin == null
              ? newData.today.dataPerHour[key].temp
              : Math.min(
                  newData.today.tempMin,
                  newData.today.dataPerHour[key].temp
                )
          newData.today.tempMax =
            newData.today.tempMax == null
              ? newData.today.dataPerHour[key].temp
              : Math.max(
                  newData.today.tempMax,
                  newData.today.dataPerHour[key].temp
      ...
      this.forecast.lastFetched = Moment(response.headers.Date).toDate()
    } catch (e) {
      console.log(e)
    }
